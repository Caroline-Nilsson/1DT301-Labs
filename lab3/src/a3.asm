...
;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
; enable external interrupts on PIND0 and PIND1
;<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
ldi temp, (3<<int0)
out EIMSK, temp
...
; calls subroutine blink_left/blink_right depending on
; lightStatus state
...
main_loop:
	...
	sbrs lightStatus, TURN_LEFT		;if TURN_LEFT bit is clear
		sbr ledState, 0b1100_0000	;set back lights bits left

	sbrs lightStatus, TURN_RIGHT	;if TURN_RIGHT bit is clear
		sbr ledState, 0b0000_0011	;set back lights bits right
...
interrupt_right:
	...
	eor lightStatus, xorComparison		;TURN_RIGHT bit toogle
...
interrupt_left:
	...
	eor lightStatus, xorComparison		;TURN_LEFT bit toogle
